...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <!--Validation controls
        ...
        ControlToValidate: which input controlthe validate belongs to
        SetFocusOnError: places cursor on the field/control if invalid
        ForeColor: message color
    ...
    <asp:RequiredFieldValidator ID="RequiredFieldStreetAddress1" runat="server" ErrorMessage="Street Address1 is Required" 
    ...
    <%-- ValidationExpression holds the pattern for the input string. Data has to match --%>
    <asp:RegularExpressionValidator ID="RegularExpressionPostalCode" runat="server" ErrorMessage="Invalid Postal Code (T6T6T6)" 
    ...
    <asp:RegularExpressionValidator ID="RegularExpressionEmailAddress" runat="server" ErrorMessage="Invalid EmailAddress" 
        ControlToValidate="EmailAddress" SetFocusOnError="true" ForeColor="Firebrick" Display="None" 
        ValidationExpression="^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$"></asp:RegularExpressionValidator>

        <asp:CompareValidator ID="CompareCheckAnswer" runat="server" ErrorMessage="Still testing incorrect answer" ControlToValidate="CheckAnswer"
        SetFocusToError="true" ForeColor="Firebrick" Display="None" Operator="Equal" ValueToCompare="15" Type="Integer"></asp:CompareValidator>

  <%-- 
    MininiumValue: lowest inclusive value of the range
        MaximumValue: lowest inclusive value of the range
        Type: specify the data type of the value

    <asp:RangeValidator ID="RangeSomeField" runat="server" ErrorMessage="Some field value is out of range (0-100)" ControlToValidate="SomeField"
        SetFocusOnError="true" ForeColor="Firebrick" Display="None" MinimumValue="0.0" MaximumValue="100.0" Type="Double"></asp:RangeValidator> 


   
    Treat the CompareValidator like an IF STATEMENT
                If(Field Operator Value)
                    --FIELD: controlToValidate
                    --OPERATOR: relative operators: >,>=, <,=, etc
                    --VALUE

    Version 1: DataType check, use to ensure you have the correct data type regardless of the value
    Version 2: constant value check, used to ensure the value entered is the the expected value
    Version 3: check against another field, used to confirm the value of field A compared to the value of field B

    <asp:CompareValidator ID="CompareVersion1" runat="server" ErrorMessage="Version 1 is a wrong data type" ControlToValidate="Version1"
        SetFocusToError="true" ForeColor="Firebrick" Display="None" Operator="DataTypeCheck" Type="Date"></asp:CompareValidator>
   
    <asp:CompareValidator ID="CompareVersion2" runat="server" ErrorMessage="Version 2 has an incorrect value" ControlToValidate="Version2"
        SetFocusToError="true" ForeColor="Firebrick" Display="None" Operator="GreaterThan" ValueToCompare="0.00" Type="Currency"></asp:CompareValidator>

    <asp:CompareValidator ID="CompareVersion3" runat="server" ErrorMessage="Version 3 value is not confirmed" ControlToValidate="Version3"
        SetFocusToError="true" ForeColor="Firebrick" Display="None" Operator="Equal" ControlToCompare="AnotherField" Type="String"></asp:CompareValidator>--%>


    <!--validation summary to display validation errors-->
    <div class="row">
